...
;                    $iRound - [optional] rounding level of label values
; =======================================================================================
Func _GraphGDIPlus_Set_RangeY(ByRef $aGraphArray, $iLow, $iHigh, $iYTicks = 1, $bLabels = 1, $iRound = 0) ;IESV Edition
	If IsArray($aGraphArray) = 0 Then Return
	...
	For $i = 1 To $iYTicks + 1
		...
		$ahTicksLabelsY[$i] = GUICtrlCreateLabel("", $aGraphArray[2] - 70, _
				($aGraphArray[3] + $aGraphArray[5]) - (($aGraphArray[5] / $iYTicks) * ($i - 1)) - 6, 60, 13, 2)
		GUICtrlSetBkColor(-1, -2)
	Next
	;----- if labels are required, then fill -----
	If $bLabels = 1 Then
...
Func _GraphGDIPlus_SaveImage($file, $hWnd)
	Local $hDC, $memBmp, $memDC, $hImage, $w, $h, $size
